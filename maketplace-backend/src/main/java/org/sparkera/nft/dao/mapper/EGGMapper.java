...
@Mapper
public interface EGGMapper extends NFTBaseMapper{
    @Update("update ts_nft_egg_info set views=views+1 where token_id=#{tokenId}")
    ...
    @Select("<script>" +
                ...
                "from ts_selling_order b left join ts_nft_egg_info a on a.token_id = b.tokenId and b.nft_type = 5 " +
                "where b.nft_type = 5 and b.ownerAddress = a.owner_address " +
    ...
    @Select("<script>" +
            ...
            "   (select count(1) from ts_favorite_info f where f.token_id = a.token_id and user_address=#{dto.address} and f.nft_type=5 )as isFavorite," +
            "   IFNULL(b.tokenId,0) as isSelling " +
            "from ts_nft_egg_info a left join ts_selling_order b on a.token_id = b.tokenId and b.nft_type = 5 " +
    ...
    @Select("<script>" +
            ...
            "   IFNULL(b.tokenId,0) as isSelling " +
            "from ts_selling_order b left join ts_nft_egg_info a on a.token_id = b.tokenId and b.nft_type = 5 " +
            "where owner_address = #{dto.address} and b.nft_type=5 " +
            "   <if test='dto.searchKey != null and dto.searchKey != \"\" ' > " +
            "   and (a.token_id like concat('%',#{dto.searchKey},'%') or lower(a.name) like concat('%',lower(#{dto.searchKey}),'%') )" +
            "   </if>" +
            "   <if test='dto.type != null ' > " +
    ...
    @Select("<script>" +
            ...
            "   (select count(1) from ts_favorite_info f where f.token_id = a.token_id and user_address=#{dto.address} and f.nft_type=5 )as isFavorite," +
            "   IFNULL(b.tokenId,0) as isSelling " +
            "from  ts_favorite_info c left join ts_nft_egg_info a on a.token_id = c.token_id and c.nft_type = 5 " +
            "  left join ts_selling_order b on a.token_id = b.tokenId and c.nft_type = b.nft_type  " +
            "where c.user_address = #{dto.address} and b.nft_type=5 " +
            "   <if test='dto.searchKey != null and dto.searchKey != \"\" ' > " +
            "   and (a.token_id like concat('%',#{dto.searchKey},'%') or lower(a.name) like concat('%',lower(#{dto.searchKey}),'%') )" +
            "   </if>" +
            "   <if test='dto.type != null ' > " +
    ...
    @Select("<script>" +
            "select token_id, owner_address, views, favorites, itemData, tokenURI, name, image, detail_image," +
            "   description, version, releaseVersion, type, last_trade_price, last_trade_time, b.price," +
            "   (select count(1) from ts_favorite_info f where f.token_id = a.token_id and user_address=#{address} and f.nft_type=5 )as isFavorite," +
            "   IFNULL(b.tokenId,0) as isSelling " +
            "from ts_nft_egg_info a left join ts_selling_order b on a.token_id = b.tokenId and b.nft_type = 5 " +
    ...
    @Select("<script>" +
            "select token_id as tokenId, owner_address as ownerAddress, count(b.tokenId) as ct, views, favorites, itemData, tokenURI, name, image, detail_image," +
            "   description, version, releaseVersion, type, last_trade_price, last_trade_time, b.price," +
            "   IFNULL(b.tokenId,0) as isSelling " +
            "from ts_nft_egg_info a left join ts_selling_order b on a.token_id = b.tokenId and b.nft_type = 5 " +
            "where a.token_id = #{tokenId} " +
    ...
    @Update("update ts_nft_egg_info set favorites =favorites + #{ct} where token_id =#{tokenId} ")
    int updateNftFavorites(@Param("tokenId")BigInteger tokenId, @Param("ct")int ct);

    @Select("<script>" +
            "select token_id, owner_address, views, favorites, itemData, tokenURI, name, image, detail_image," +
            "   description, version, releaseVersion, type_id, type, last_trade_price, last_trade_time " +
            "from ts_nft_egg_info " +
            "where token_id = #{tokenId}" +
            "</script>")
    NftEggInfoDto getNftInfoByTokenId(@Param("tokenId")BigInteger tokenId);

    @Update("replace into ts_nft_egg_info" +
            "   (token_id, owner_address, views, favorites, itemData, tokenURI, name, image, detail_image, description, version," +
            "   releaseVersion, type_id, type, last_trade_price, last_trade_time)" +
            ...
            "   #{dto.typeId}, #{dto.type}, #{dto.lastTradePrice}, #{dto.lastTradeTime})")
    int upInsetNftInfo(@Param("dto") BaseEntityInfo nftEggInfoDto);

    @Update("<script>" +
            " update ts_nft_egg_info set last_trade_price =#{price}" +
