...
import org.sparkera.nft.dao.dto.NftEgseInfoDto;
...
@Mapper
public interface EGSEMapper extends NFTBaseMapper{
    @Update("update ts_nft_egse_info set views=views+1 where token_id=#{tokenId}")
    ...
    @Select("<script>" +
            ...
            " a.`views`, a.`favorites`,(select count(1) from ts_favorite_info f where f.token_id = a.`token_id` and user_address=#{dto.address} and f.nft_type=3 )as isFavorite, " +
            " b.`price` " +
            " from  ts_selling_order b left join ts_nft_egse_info a  on a. `token_id` = b.tokenId and b.nft_type=3 " +
            " where b.nft_type=3 and b.ownerAddress = a.owner_address " +
    ...
    @Select("<script>" +
            ...
            "   (select count(1) from ts_favorite_info f where f.token_id = a.`token_id` and user_address=#{dto.address} and f.nft_type=3 )as isFavorite " +
            "        from ts_nft_egse_info a   left join ts_selling_order b on a. `token_id` = b.tokenId and b.nft_type=3" +
    ...
    @Select("<script>" +
            ...
            "     `views`, `favorites` ,IFNULL(b.tokenId,0) as isSelling," +
            "   (select count(1) from ts_favorite_info f where f.token_id = a.`token_id` and user_address=#{dto.address} and f.nft_type=3 )as isFavorite " +
            "        from ts_selling_order b left join ts_nft_egse_info a  on a. `token_id` = b.tokenId \n" +
            "        where b.`ownerAddress` = #{dto.address} and b.nft_type=3" +
            "   <if test='dto.searchKey != null and dto.searchKey != \"\" ' > " +
            "   and  (a.`token_id` like concat('%',#{dto.searchKey},'%') or lower(a.`name`) like concat('%',lower(#{dto.searchKey}),'%') )" +
            "   </if>" +
            "   <if test='dto.type != null ' > " +
    ...
    @Select("<script>" +
            ...
            "   `name`, `image`, `description`, `attr_type` as attrType,b.`price` ," +
            "     `views`, `favorites` ,IFNULL(b.tokenId,0) as isSelling " +
            "   from ts_favorite_info c left join ts_nft_egse_info a  on a.`token_id` = c.token_id " +
            "   left join ts_selling_order b on a.`token_id` = b.tokenId and c.nft_type=b.nft_type " +
            "    where c.nft_type = 3 and c.`user_address` = #{dto.address}" +
            "   <if test='dto.searchKey != null and dto.searchKey != \"\" ' > " +
            "   and  (a.`token_id` like concat('%',#{dto.searchKey},'%') or lower(a.`name`) like concat('%',lower(#{dto.searchKey}),'%') )" +
            "   </if>" +
            "   <if test='dto.type != null ' > " +
    ...
    @Select("select `token_id`as tokenId, `owner_address` as ownerAddress, 3 as nftType,`type`, " +
            "   `name`, `detail_image` as image, `description`, `attr_type` as attrType,b.`price` ," +
            "    IFNULL(b.tokenId,0) as isSelling ,`views`, `favorites`," +
            "  (select count(1) from ts_favorite_info c where c.token_id=a.`token_id` and c.user_address=#{address} and c.nft_type=3) as isFavorite " +
            " from ts_nft_egse_info a left join ts_selling_order b on a. `token_id` = b.tokenId and b.nft_type=3 " +
            "        where `token_id` = #{tokenId}")
    Map<String, Object> tokenDetail(@Param("tokenId")BigInteger tokenId,@Param("address") String address);
    @Select("select a.`token_id`as tokenId, a.`owner_address` as ownerAddress,count(b.tokenId) as ct,type from ts_nft_egse_info a " +
            " left join ts_selling_order b on a. `token_id` = b.tokenId and b.nft_type=3     where a.`token_id` = #{tokenId}")
    Map<String, Object> checkToken(@Param("tokenId")BigInteger tokenId);
    @Update("update ts_nft_egse_info set favorites =favorites + #{ct} where token_id =#{tokenId} ")
    ...
    @Select("<script>" +
            ...
            "   `last_trade_price`, `last_trade_time`, `detail_image` from ts_nft_egse_info " +
            " where `token_id` =#{tokenId}" +
            "</script>")
    NftEgseInfoDto getNftInfoByTokenId(@Param("tokenId")BigInteger tokenId);
    @Update(" replace into `ts_nft_egse_info`" +
    ...
    int upInsetNftInfo(@Param("dto")BaseEntityInfo nftEgseInfoDto);
    @Update("<script>" +
            " update ts_nft_egse_info set last_trade_price =#{price}" +
