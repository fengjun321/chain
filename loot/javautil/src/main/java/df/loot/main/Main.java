...
import df.loot.parser.ExcelParser;
import df.sd.Counter;
...
public class Main {
	public static void main(String[] args) throws IOException, TemplateException, ParseException {
		...
		Template template = configuration.getTemplate("loot_tpl_split.ftl");// 可以是<相对路径>，也可以是<绝对路径>
		// 5、创建模板文件需要展示数据的数据集对象，可以使用POJO，也可以使用map 一般是使用map
		Map<String, Object> model = new HashMap<>();
		
		 
		List<Trait> readTrait = ExcelParser.readTrait();
		
		Counter counter=new Counter();
		model.put("traitList", readTrait);
		model.put("seed", counter);		
		
		String pre_file_path = "./src/main/resources";
		// 6、创建一个FileWriter对象 指定生成的静态文件的文件路径及文件名
		// 拼接一个前缀和后缀
		FileWriter writer = new FileWriter(new File(pre_file_path + "/loot.html"));
